...
"""
fmri_network_analysis_negedges.py

Purpose:
    Computes graph-theory network metrics from subject-level resting-state
    functional connectivity (FC) matrices using NEGATIVE edges only
    (positive correlations set to zero; negative values then absolutised;
    diagonal set to zero).

    This is the negative-edges sensitivity analysis. Results from this version
    were reported in the supplementary materials of the associated paper.
    Main text results used absolute edges
    (see fmri_network_analysis_absedges.py).

    Note: eigenvector centrality uses max_iter=200 (increased from the default
    of 100) to handle subjects where power iteration failed to converge.
...
Usage:
    Update the USER CONFIGURATION section below, then run:
        python fmri_network_analysis_negedges.py
    Or submit via SLURM:
...
output_dir = "/path/to/HCA_stat/network_negonly"
...
for subject in subj_list:
    ...
    # ── Prepare matrix: set diagonal to 0, retain negative edges only ─────────
    ...
    # Negative-only: set positive correlations to zero, then absolutise
    matrixnegonly = matrixdiag0.copy()
    matrixnegonly[matrixnegonly > 0] = 0
    matrixnegonly = abs(matrixnegonly)

    # ── Build graph ───────────────────────────────────────────────────────────

    G = nx.from_numpy_array(matrixnegonly)
    ...
    # max_iter increased from default (100) to handle subjects where power
